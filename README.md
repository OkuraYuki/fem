...
<エントリ数>
[材料番号] <電位値>           （従来形式）
[XMIN|XMAX|YMIN|YMAX|ZMIN|ZMAX] <電位値>
[FACE 面名 電位値]             （例: FACE XMIN 1.0）
```

- 材料番号指定: 指定した材料番号に属する全節点を固定電位として登録する。
- 面指定: メッシュ外接直方体の各面（XMIN/XMAX/YMIN/YMAX/ZMIN/ZMAX）上の節点を固定電位として登録する。
