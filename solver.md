...
目的:
- `dt > 0` の過渡解析で、線形ソルバーの残差が収束判定値へ近づきにくい問題を改善する。
- 特に model_11 のように導電率の差が非常に大きいモデルで、SSOR-CG ではなく ICCG を使って安定性を優先する。

今回の実装内容:

1. **過渡ステップ行列の作り直し**
   - RCM後の `K_sigma` を `base_K_*` として保存する。
   - 各時間ステップの最初に `global_K_*` を `base_K_*` へ戻してから、
     `A = K_sigma + K_epsilon / dt` を作る。
   - これにより、`K_epsilon / dt` がステップごとに追加され続ける問題を防ぐ。

2. **Dirichlet条件で変更された行列を次ステップへ持ち越さない**
   - `solve_linear_system()` は固定節点の行・列処理と対角スケーリングを `global_K_*` に直接反映する。
   - そのため、過渡解析では毎ステップ `base_K_*` から復元することで、
     前ステップで埋め込んだ境界条件やスケーリングの影響をリセットする。

3. **出力節点順の補正**
   - 計算中の `potentials` は RCM 後の節点順。
   - `tempa.datXXX` を書く直前だけ入力メッシュの節点順へ戻して出力する。
   - 計算内部はRCM順のまま保つため、ソルバー処理には影響しない。

4. **どんな状況でもICCGを使用**
   - 大規模過渡解析でも SSOR-CG へ切り替えない。
   - `use_ssor = false` とし、常に IC(0) 前処理付きCG、つまり `ICCG (scaled)` を使う。
   - model_11 のような強い材料コントラストでは、軽いSSORよりICCGの方が残差を下げやすい。
   - 過渡解析の反復上限は `max(1000, nstep * 10)` とし、短いステップ数のテストでも50回程度で早く打ち切らない。

5. **対称対角スケーリングを過渡解析にも適用**
   - 以前は主に定常解析向けだったスケーリングを、定常・過渡の両方で使う。
   - `A_s = S A S`, `b_s = S b`, `phi = S y` の形に変換し、CGの対称性を保ったまま係数の桁差を小さくする。
   - 固定節点の値もスケーリング後の未知数に合わせて `fixed_value / scale` として扱う。

読むときのポイント:
- `solve()` の過渡ループでは、各ステップで `base_K_*` から `global_K_*` を復元しているかを見る。
- `build_time_step_matrix(1.0 / dt, ...)` は、復元済みの `K_sigma` と `K_epsilon` からそのステップ専用の行列を作る。
- `solve_linear_system()` では、SSOR分岐は残っているが `use_ssor` が常に `false` なので ICCG が選ばれる。
- `analysis_summary.txt` の `solver_method` は、正常なら `ICCG (scaled)` と出る。

注意:
- `ddmin` の読み込みは今回の修正対象外。現在の時間ステップ間収束判定は `conv_tol = 1e-8`、過渡線形ソルバーの相対残差許容値は `1e-10`。
- ICCGはSSORより前処理構築が重いが、今回は「どんな状況でもICCGでよい」という方針に合わせて収束安定性を優先した。
